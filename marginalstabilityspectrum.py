...
def findmarginalomega(Rayleigh, Prandtl,Nz, ad, sig,Lz):
    ...
    #marginal Rayleigh
    #     # print("Rayleigh={}".format(ra_guess))
    #     print('\n\n'+'Iteration #:', str(counter)+'\n\n' )
    #     print("omeg_guess={}".format(omeg_guess))
    # Found marginal stability
    #Printing final rates
    #Rootfinding
    def getgrowthRa(arg_Ra):
        print(arg_Ra.type())
        maxgrowthratessolve = max(getgrowthrates(arg_Ra, Prandtl,Nz, ad, sig,Lz))
        comm.barrier()
        if rank == 0:
            print('Rayleigh: ',arg_Ra)
            print('Growth rate: ',maxgrowthratessolve)
        return abs(maxgrowthratessolve)
    # omeg_scalr = minimize_scalar(getgrowthRa, bounds=(0,20), method='bounded')
    omeg_scalr = root_scalar(getgrowthRa, x0 = Rayleigh )
    if rank == 0: 
        print('Solution:', omeg_scalr.root) 
    ra_guess = omeg_scalr.root
    omeg_guess = 0
    finalrates = getgrowthrates(ra_guess, Prandtl,Nz, ad, sig,Lz)
    if rank == 0:
    ...
    for i in range(len(finalrates)):
        ...
        if abs(omega_final - omeg_guess) < 10**(-3):
...
for i in range(len(ad_list)):
    if not (ad_list[i] == ad_list[0]):
        if rank == 0:
            print('rank:',rank)
            print('###')
        margRa = findmarginalomega(marginalRa[i-1],Prandtl,ad_list[0],Nz,sig,Lz)[0]
...
print(ad_list)
print(marginalRa)
plt.plot(ad_list,marginalRa)
full_dir = path + '/eigenvalprob_plots/marginalstabilityconditions/'
plt.savefig(full_dir+'marginallystablecurve_sig{}Nz{}'.format(sig,Nz)+'.png')
plt.close()