...
growth_global, freq_global = getgrowthrates(ra_sig, Prandtl, Nz, ad, sig,Lz,Re_arg)
if comm.rank == 0:
    #Plotting Set-up
    fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(11,9), sharex=True, dpi = 500)
    fig.suptitle(r'$Ra_{marginal}$='+f" {(Rayleigh):.3e}"+r' $\nabla_{ad}$'+'= {} '.format(ad)+r'$\sigma$'+'= {} '.format(sig)+r'$S_{c}$'+'= {}'.format(supercrit)+' Re= {}'.format(Re_arg))
    ax1.set_ylabel(r'$\omega$')
    ax2.set_ylabel(r'$\text{f}$')
    ax2.set_xlabel(r'$k_x$')
    if not(kxbool):
        ax2.set_xscale('log')
    ax1.title.set_text(r'Rayleigh-Benard Modes Growth Rates ($\mathrm{Ra_{f}} = %.3e, \; \mathrm{Pr} = %.2f$)' %(ra_sig, Prandtl))
    ax2.title.set_text(r'Rayleigh-Benard Modes Frequency($\mathrm{Ra_{f}} = %.3e, \; \mathrm{Pr} = %.2f$)' %(ra_sig, Prandtl))
    ...
    if solvebool:
        filename = 'ad{}'.format(ad)+'sig{}'.format(sig)+'Ra{}'.format(Rayleigh)+'sprcrt'+f'{(supercrit)}'+'Re{}'.format(Re_arg)+'SPARSE_eigenval_plot.png'
    else:
        filename = 'ad{}'.format(ad)+'sig{}'.format(sig)+'Ra{}'.format(Rayleigh)+'sprcrt'+f'{(supercrit)}'+'Re{}'.format(Re_arg)+'DENSE_eigenval_plot.png'
